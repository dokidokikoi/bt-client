...
func checkIntgerity(pw *pieceWork, buf []byte) error {
	hash := sha1.Sum(buf)
...
func (t *Torrent) startDownloadWorker(peer peers.Peer, workQueue chan *pieceWork, result chan *pieceResult) {
	...
	if err != nil {
		log.Printf("Could not handshake with %s, Disconnecting. err %s\n", peer.Ip, err.Error())
		return
	}
	defer c.Conn.Close()
...
func (t *Torrent) Download() ([]byte, error) {
	...
	for donePieces < len(t.PieceHashes) {
		...
		numWorkers := runtime.NumGoroutine() - 1 // 排除主线程
